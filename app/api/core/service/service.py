...
CATALOG: dict[str, EmployeeCatalogItem] = {
    ...
    "content": EmployeeCatalogItem("content", "Content Employee", "Content Creator", "Creates governed product content; publishing remains subject to configured adapters.", frozenset({"content", "product_knowledge"}), frozenset({"no_fake_claims", "no_unverified_publish"}), frozenset({"READ_PRODUCT", "READ_KNOWLEDGE", "CREATE_CONTENT"}), frozenset({"FREE", "STARTER_199", "TEAM_399", "DEPARTMENT_699", "ORGANIZATION_1499"})),
...
class PackageEngine:
    """Single source of truth for package, entitlement and quota policy."""
    def __init__(self, packages: Mapping[str, Package] | None = None):
        self.packages = dict(packages or PACKAGES)
    ...
    def has_feature(self, package_id: str, feature: str) -> bool:
        return feature in self.get(package_id).entitlements.get("features", set())
...
class QuotaEngine:
    ...
    def state(self, organization_id: str, resource: str) -> QuotaState:
        ...
        used = self.used.get((organization_id, resource), 0)
        if used > limit:
...
class UsageMeter:
    ...
    def record(self, organization_id: str, resource_type: str, quantity: int = 1, employee_id: str | None = None, task_id: str | None = None) -> UsageEvent:
        ...
        event = UsageEvent(sha256(raw).hexdigest()[:16], organization_id, employee_id, task_id, resource_type, quantity, time())
        self.events.append(event)
        return event

    def total(self, organization_id: str, resource_type: str) -> int:
        return sum(e.quantity for e in self.events if e.organization_id == organization_id and e.resource_type == resource_type)


class BillingProvider:
    """Boundary only. No provider means no payment/subscription success claim."""
    configured = False

    def create_subscription(self, organization_id: str, package_id: str) -> BillingStatus:
        return BillingStatus.NOT_CONFIGURED

    def cancel_subscription(self, billing_reference: str) -> BillingStatus:
        return BillingStatus.NOT_CONFIGURED

    def verify_webhook(self, payload: bytes, signature: str, secret: str | None) -> bool:
        if not secret:
            return False
        expected = sha256(secret.encode() + payload).hexdigest()
...
class ServiceEngine:
    def __init__(self, store: TenantStore | None = None, billing: BillingProvider | None = None):
        ...
        self.billing = billing or BillingProvider()
        self.packages = PackageEngine()
        self.subscriptions: dict[str, Subscription] = {}
        self.contracts: dict[str, EmployeeContract] = {}
        self.usage = UsageMeter()
        self.quota = QuotaEngine({"tasks": 100, "messages": 500, "content": 100, "posts": 20, "employees": 2, "storage": 1000, "integrations": 2})
    ...
    def _audit(self, organization_id: str, actor: str, action: str, result: str, reference: str | None = None) -> None:
        raw = f"{organization_id}|{actor}|{action}|{result}|{reference}|{len(self.audit)}".encode()
        self.audit.append(AuditEvent(sha256(raw).hexdigest()[:16], organization_id, actor, action, result, time(), reference))

    def create_organization(self, customer_id: str, organization_id: str) -> dict[str, Any]:
        self.store.organizations[organization_id] = {"customer_id": customer_id, "status": "ACTIVE"}
        self._audit(organization_id, customer_id, "organization_created", "ACTIVE")
        return dict(self.store.organizations[organization_id])

    def start_trial(self, organization_id: str, package_id: str = "FREE", *, idempotency_key: str | None = None) -> Subscription:
        if idempotency_key and idempotency_key in self._idempotency:
            return next(s for s in self.subscriptions.values() if s.organization_id == organization_id)
        self.packages.get(package_id)
        now = time()
        subscription = Subscription(f"sub_{len(self.subscriptions)+1}", organization_id, package_id, SubscriptionStatus.TRIAL, now, trial_end_at=now + 14 * 86400)
        self.subscriptions[subscription.subscription_id] = subscription
        if idempotency_key: self._idempotency.add(idempotency_key)
        self._audit(organization_id, "system", "subscription_created", subscription.status.value, subscription.subscription_id)
        return subscription
    ...
    def activate_subscription(self, subscription_id: str, *, payment_authorized: bool, idempotency_key: str | None = None) -> Subscription:
        ...
        if not payment_authorized:
            raise PermissionError("PAYMENT_AUTHORIZATION_REQUIRED")
        billing_status = self.billing.create_subscription(sub.organization_id, sub.package_id)
        if billing_status != BillingStatus.ACTIVE:
            raise RuntimeError(BillingStatus.NOT_CONFIGURED.value)
        updated = Subscription(sub.subscription_id, sub.organization_id, sub.package_id, SubscriptionStatus.ACTIVE, sub.start_at, sub.renew_at, sub.cancel_at, sub.trial_end_at, "configured")
        self.subscriptions[subscription_id] = updated
        if idempotency_key: self._idempotency.add(idempotency_key)
        self._audit(sub.organization_id, "customer", "subscription_changed", "ACTIVE", subscription_id)
        return updated
    ...
    def change_package(self, subscription_id: str, package_id: str, *, authorized: bool, idempotency_key: str | None = None) -> Subscription:
        ...
        self.packages.get(package_id)
        current_employees = sum(1 for c in self.contracts.values() if c.organization_id == old.organization_id and c.status in {EmployeeContractStatus.ACTIVE, EmployeeContractStatus.TRIAL})
        limit = self.packages.get(package_id).employee_limit
        if limit is not None and current_employees > limit:
            raise ValueError("DOWNGRADE_BLOCKED_EMPLOYEE_LIMIT")
        updated = Subscription(old.subscription_id, old.organization_id, package_id, old.status, old.start_at, old.renew_at, old.cancel_at, old.trial_end_at, old.billing_reference)
        self.subscriptions[subscription_id] = updated
        if idempotency_key: self._idempotency.add(idempotency_key)
        self._audit(old.organization_id, "customer", "package_changed", package_id, subscription_id)
        return updated
    ...
    def cancel(self, subscription_id: str, *, immediate: bool, authorized: bool) -> Subscription:
        ...
        old = self.subscriptions[subscription_id]
        status = SubscriptionStatus.CANCELED if immediate else old.status
        updated = Subscription(old.subscription_id, old.organization_id, old.package_id, status, old.start_at, old.renew_at, time() if immediate else old.cancel_at, old.trial_end_at, old.billing_reference)
        self.subscriptions[subscription_id] = updated
        self._retention[old.organization_id] = "RETENTION"
        self._audit(old.organization_id, "customer", "cancellation", status.value, subscription_id)
        return updated
    ...
    def hire_employee(self, organization_id: str, employee_kind: str, *, permissions: frozenset[str], configuration: Mapping[str, Any] | None = None) -> EmployeeContract:
        ...
        employee_id = f"{organization_id}:{employee_kind}:{count+1}"
        employee = Employee(employee_id, organization_id, item.role, permissions, EmployeeStatus.CONFIGURED)
        self.store.add_employee(employee)
        status = EmployeeContractStatus.TRIAL if sub.status == SubscriptionStatus.TRIAL else EmployeeContractStatus.ACTIVE
        contract = EmployeeContract(f"contract_{len(self.contracts)+1}", employee_id, organization_id, sub.package_id, time(), sub.renew_at, permissions, {"tasks": self.quota.limits["tasks"]}, status, dict(configuration or {}))
        self.contracts[contract.contract_id] = contract
        self._audit(organization_id, "customer", "employee_hired", status.value, contract.contract_id)
        return contract
    ...
    def record_usage(self, organization_id: str, resource: str, quantity: int = 1, *, employee_id: str | None = None, task_id: str | None = None) -> QuotaState:
        ...
        if state == QuotaState.EXCEEDED:
            self._audit(organization_id, "system", "quota_reached", state.value, resource)
            return state
    ...
    def dashboard(self, organization_id: str) -> dict[str, Any]:
        self.store._check_org(organization_id, organization_id)
        employees = [c for c in self.contracts.values() if c.organization_id == organization_id]
        subs = [s for s in self.subscriptions.values() if s.organization_id == organization_id]
    ...
    def readiness(self, *, phase44_ready: bool, social_configured: bool, billing_configured: bool) -> dict[str, Any]:
        base = {"employee_deployment": phase44_ready, "tenant_isolation": phase44_ready, "task_execution": phase44_ready, "verification": phase44_ready, "quota": True, "usage_metering": True, "subscription_state": True, "entitlement": True, "security": True, "audit": True, "customer_dashboard": True, "employee_management": True}
        free = {"FREE_COMMERCIAL_READY": all(base.values())}
        starter = {"STARTER_199_COMMERCIAL_READY": all(base.values()) and social_configured and billing_configured}
        return {**free, **starter, "TEAM_399_COMMERCIAL_READY": starter["STARTER_199_COMMERCIAL_READY"], "DEPARTMENT_699_COMMERCIAL_READY": starter["STARTER_199_COMMERCIAL_READY"], "ORGANIZATION_1499_COMMERCIAL_READY": False, "evidence": base, "billing": BillingStatus.ACTIVE.value if billing_configured else BillingStatus.NOT_CONFIGURED.value}
